...
class Features:
    ...
    '''
    def __init__(self, list_of_fe=None):
        self.list_of_fe = list_of_fe
    
    def name(self):
        if self.list_of_fe is None:
            return "RAW"
        name = ""
...
class TranslatedHeight:
    '''
    translate the height by the mean height of the catchment, then rescale
    '''
    def __init__(self, scale=1,no_data=0):
        self.scale = scale
        self.no_data=0
    ...
    def features(self, dem_array):
        dem_array=np.copy(dem_array)# make a copy
        
        mask_no_data = dem_array < 0
        mask_has_data= dem_array >= 0
        # dem start from 0
        dem_array-=dem_array[mask_has_data].min()
        # set no-data value
        dem_array[mask_no_data]=self.no_data
        return (self.scale * dem_array)[...,np.newaxis]

class ReverseTranslatedHeight:
    '''
    translate the height by the mean height of the catchment, then rescale
    '''
    def __init__(self, scale=0.01,no_data=0):
        self.scale = scale
        self.no_data=0
    ...
    def features(self, dem_array):
        ...
        mask_has_data= dem_array >= 0
        # dem start from 0
        dem_array=dem_array.max()-dem_array
        # set no-data value
        dem_array[mask_no_data]=self.no_data
        return (self.scale * dem_array)[...,np.newaxis]
...
class Mask:
    def __init__(self, on_val=1,off_val=0):
        self.on_val=on_val
    ...
    def features(self, dem_array):
        ...
        '''
        # mask
        mask_array = np.ones_like(dem_array,dtype=np.float32)*self.on_val
        mask_array[dem_array<0] = self.off_val
        return mask_array[...,np.newaxis]

class AsCategory:
    '''
    convert the float number to integer (category)
    '''
    def __init__(self, min_val, max_val, band_num):
        ...
        e.g. min = 0, max = 1, band = 2, will get step = 0.5, band_0 = 0 to 0.5, band_1 = 0.5 to 1
        '''
    ...
    def name(self):
        return "C" + str(self.band_num)
        
    def channels(self):
        return self.band_num
        
    def features(self, dem_array):
        return np.floor((np.clip(dem_array, self.min_val, self.max_val-self.step*0.001) - self.min_val) / self.step)[...,np.newaxis]

class AsCategoryFloat:
    '''
    convert the float number to category
    '''
    def __init__(self, min_val, max_val, band_num):
        '''
        min value (included)
    ...
    def name(self):
        return "Cf" + str(self.band_num)
        
    def channels(self):
        return self.band_num
        
    def features(self, dem_array):
        return ((np.clip(dem_array, self.min_val, self.max_val-self.step*0.001) - self.min_val) / self.step)[...,np.newaxis]
...
class OneHot:
    ...
    def features(self, dem_array):
        ...
        return (np.arange(self.band_num) == dem_int[...,None]).astype(np.float32)
